...
function ArmaEnsalada({ variant = 'lista', selecciones, onToggle, arma, kind = 'ensalada' }) {
  const armaData = arma || window.MENU_DATA.arma;

  if (variant === 'cards') return <ArmaCards arma={armaData} sel={selecciones} onToggle={onToggle} kind={kind} />;
  return <ArmaLista arma={armaData} sel={selecciones} onToggle={onToggle} kind={kind} />;
}
...
function ArmaLista({ arma, sel, onToggle, kind = 'ensalada' }) {
  const totalSel = arma.pasos.reduce((a, p) => a + (sel[p.id] || []).length, 0);
  const titulo = kind === 'comida' ? 'Tu comida' : 'Tu ensalada';
  const ingrLabel = kind === 'comida' ? 'Empezá a armarla' : 'Empezá a armarla';

  return (
    <div>
      {/* Resumen */}
      <div className="pv-card" style={{ background: 'var(--terracota-soft)', borderColor: 'transparent', marginBottom: 14 }}>
        <div className="pv-eyebrow" style={{ color: 'oklch(0.42 0.1 40)' }}>{titulo}</div>
        <div className="pv-h3" style={{ fontSize: 22, marginTop: 4, color: 'oklch(0.32 0.08 40)' }}>
          {totalSel === 0 ? ingrLabel : `${totalSel} ${totalSel === 1 ? 'ingrediente' : 'ingredientes'}`}
        </div>
        {totalSel > 0 && (
              ...
              return (
                ...
                >
                  ...
                  {op.precio > 0 && (
                    ...
                    }}>+{window.formatPrecio(op.precio)}</span>
                  )}
                </button>);
...
function ArmaCards({ arma, sel, onToggle, kind = 'ensalada' }) {
  const titulo = kind === 'comida' ? 'Arma tu comida' : 'Arma tu ensalada';
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        <div className="pv-eyebrow">{titulo}</div>
                  ...
                  >
                    ...
                    <div style={{ fontSize: 13, fontWeight: 500, lineHeight: 1.2 }}>
                      ...
                      {op.precio > 0 && (
                        ...
                        }}>+{window.formatPrecio(op.precio)}</div>
                      )}
                    </div>
                  </button>
                );
